...
library CPAMMUtils {
    ...
    uint256 internal constant MAX_FEE = 100_000; // 10%
    ...
    error InsufficientLiquidity(uint256 reserveA, uint256 reserveB, uint256 minLiquidity);

    // Core calculation functions
    function calculateReservesFromSqrtPrice(uint160 sqrtPriceX96)
        internal
        pure
    ...
    function calculateInitialLiquidity(uint256 amount0, uint256 amount1) internal pure returns (uint256) {
    ...
    function hasLiquidity(uint256 amount0, uint256 amount1) internal pure returns (bool) {
        return amount0 >= MIN_LIQUIDITY && amount1 >= MIN_LIQUIDITY;
    }

    function validatePool(address factory, PoolId poolId) internal view returns (bool) {
    ...
    function calculateK(uint256 reserve0, uint256 reserve1) internal pure returns (uint256) {
        return reserve0 * reserve1;
    }

    function validateK(uint256 oldK, uint256 newK) internal pure returns (bool) {
        return newK >= oldK;
    }
