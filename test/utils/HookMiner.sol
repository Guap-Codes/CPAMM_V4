...
library HookMiner {
    ...
    /// @return salt The salt that produces an address with the desired flags
        ...
        returns (bytes32 salt, address hookAddress)
    {
        ...
        uint256 saltNum;
        for (saltNum; saltNum < MAX_LOOP; saltNum++) {
            salt = bytes32(saltNum);
            hookAddress = computeAddress(deployer, saltNum, creationCodeWithArgs);
